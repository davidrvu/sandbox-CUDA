...
int main ( int argc , char** argv ){
	...
	strcat(file_name_steps, matrix_size_str);
	strcat(file_name_steps, "_float.bin");
	strcat(file_name, file_name_steps);
	printf("file_name = %s \n", file_name);
	//char file_name[80] = "/home/david/matrix_data_base/mat_5x5_float.bin"; 

	//magma_int_t m = 8192; // mxm matrix
	//magma_int_t m = 4096; // mxm matrix
	//magma_int_t m = 20000;      // mxm matrix
	//magma_int_t m;      // mxm matrix
	...
	magma_smalloc_cpu(&h_mat_in, mm);   
	float numero;
	int celda;
	for (celda = 0; celda < matrix_size*matrix_size; celda++){
		fread(&numero,sizeof(float),1,ptr_myfile);
    ...
    magma_ssetmatrix( m, m, h_mat_in, m, d_A, m );      //Copy data FROM host TO device
